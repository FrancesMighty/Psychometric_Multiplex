...
def has_minimum_edge_count(nodes, G):
    edge_count = sum(1 for u, v in combinations(nodes, 2) if G.has_edge(str(u), str(v)))
    return edge_count >= len(nodes)


def two_interscale_mults_combinations_G(D: dict, G: nx.Graph, orders=(3, 4, 5)):
    all_combo = {}
        ...
        for k in orders:
            ...
            valid_combos = [
                ...
                if any(i in items1 for i in combo)
                   ...
                   and has_minimum_edge_count(combo, G)
            ]
...
def two_interscale_mults_combinations_G_k(D: dict, G: nx.Graph, orders=(3, 4, 5)):
    """
    Generate inter-scale multiplets from all pairs of scales.
    ...
    for scale1, scale2 in combinations(D.keys(), 2):
        ...
        for k in orders:
            ...
            valid_combos = [
                combo
                for combo in combinations(combined_items, k)
                if (
                    any(i in items1 for i in combo)
                    and any(i in items2 for i in combo)
                    and has_minimum_edge_count(combo, G)
                )
            ]

            all_combo[k][pair_key] = valid_combos

    return all_combo


def order_interscale_mults_combinations_G(D: dict, G: nx.Graph, orders=(3, 4, 5)):
    all_combo = {}
    ...
    for k in orders:
        ...
        # Choose exactly k scales
        for scales in combinations(D.keys(), k):

            candidates = []

            # One item from each selected scale
            item_lists = [D[scale] for scale in scales]

            for combo in product(*item_lists):

                if has_minimum_edge_count(combo, G):
                    candidates.append(combo)

            all_combo[k][scales] = candidates

    return all_combo


def filter_and_sample_multiplets_DF(multiplets_dict, sample_size=400, random_state=None):

    filtered = {}

    for order, df in multiplets_dict.items():

        if order == 3 or len(df) <= sample_size:
            filtered[order] = df.copy().reset_index(drop=True)
        else:
            filtered[order] = (
                df.sample(
                    n=sample_size,
...
def multiplets_to_df(multiplets_dict):
    """
    Convert {order: pattern: list_of_tuples} to {order: DataFrame}
    where the columns are node1, ..., nodeN.
    """
    out = {}
    for order, pattern_dict in multiplets_dict.items():
        cols = [f"node{i}" for i in range(1, order + 1)]
        # Flatten all multiplets across scale patterns
        rows = [
            multiplet
            for multiplets in pattern_dict.values()
            for multiplet in multiplets
        ]
        out[order] = pd.DataFrame(rows, columns=cols)
