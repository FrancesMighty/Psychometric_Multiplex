...
def _make_u_hard(node2comm):
    ...
    Build hard one-hot membership matrix U (shape N x K) aligned to node ids [0..N-1] if possible.
            comm0  comm1  comm2
    node 0  [  0,    0,    1  ]   ← community 2
    node 1  [  1,    0,    0  ]   ← community 0 etc.
...
def _estimate_w_from_R_or_G(
    ...
    Returns:
        ...
        W[k, l] measures, on avg, how strongly nodes in comm k are corr with nodes in comm l
    -------
    ...
    num[k,l] — sum of absolute values (correlations or weights) for all pairs (i∈k, j∈l)
    den[k,l] — count of such pairs
    W = num / den
    """
    K = max(node2comm.values()) + 1
...
def _initial_cliques(
    ...
    maximal: you cannot add any further node to it and keep it a clique — not a subset of a larger clique.
    igraph uses the Bron-Kerbosch algorithm (with pivoting), a recursive backtracking method
    ...
    for C in max_cliques_idx:
        ...
        if len(C) <= k_max:
            ...
            # decompose cliques when nodes are more than k_max
            # break big maximal cliques into all k-subsets within range
            for k in range(k_min, k_max + 1):
                for subset in combinations(C, k):  # k_subsets
                    out.add(subset)
...
def score_hyperedge(
    ...
    for each hyperedge e:
        sum the pairwise community affinities between all node pairs in e
        scales by a size penalty k (in _kappa(...))
    """
    nodes = list(e)
    acc = 0.0
...
def rank_candidates(
    ...
    '''
    A hyperedge scores high if all its nodes belong to communities that
    are strongly coupled in W. The ranking then surfaces the candidates
    most supported by the community structure as the best hyperedge estimates.
    '''
...
def expand_candidates(
    ...
    """
    Takes the ranked clique-based candidates and tries to grow each one by adding neighboring nodes,
    accepting the expansion only if it meaningfully improves the score:
    ...
    for seed, s_seed in ranked:
        ...
        for u in neighs:
            ...
            if cand in seen:  # already added from another expansion
                continue
            s_cand = score_hyperedge(cand, U, W, kappa_mode)
            if s_cand >= s_seed * (1.0 + min_gain_ratio):
                print(f"seed size {len(seed)} → cand size {len(cand)}")
...
def expand_candidates(
    ...
    """
    For each original clique seed, tries to grow it by adding 1, 2, ... up to
    (max_size - len(seed)) nodes, always starting from the original seed, never
    from derived candidates, to avoid quality drift and combinatorial explosion.

    Expansion strategy:
        - At each step k, find all subsets of k neighbors to add to the seed
        - Accept only if score improves by at least min_gain_ratio over seed score
        - Keep top_per_seed best expansions per (seed, expansion_size)

    Parameters:
    -----------
    G : ig.Graph - The input graph (fully connected, weighted).
    ranked : List[Tuple[Tuple[int, ...], float]]
        The ranked list of clique-based candidates from rank_candidates().
        Each element is (candidate_tuple, score).
    U : np.ndarray - Hard membership matrix (N x K), one-hot rows from Spinglass.
    W : np.ndarray - Community affinity matrix (K x K) from _estimate_w_from_R_or_G().
    kappa_mode : str - Size penalty mode for score_hyperedge(). See _kappa().
    max_size : int - Maximum hyperedge size allowed. Seeds already at this size are skipped.
    top_per_seed : int
        How many best expansions to keep per (seed, expansion_size) pair.
        Limits branching to avoid explosion of candidates.
    min_gain_ratio : float
        Minimum relative score improvement to accept an expansion (e.g. 0.05 means at least 5% better than seed)
    max_combinations : int - Max num of neighbor subsets to evaluate per (seed, k) pair.
        If C(|neighbors|, k) exceeds this, we randomly sample max_combinations subsets
        instead of enumerating all of them. Prevents combinatorial explosion.
    """

    # ------------------------------------------------------------------ #
    # seen: tracks all candidate tuples already added to the pool - from the orig ranked list and from expansions
    # out:  the growing list of (candidate, score) pairs that we will return
    # ------------------------------------------------------------------ #
    seen: Set[Tuple[int, ...]] = {c for c, _ in ranked}
    out: List[Tuple[Tuple[int, ...], float]] = list(ranked)

    def _seed_neighbors(seed: Tuple[int, ...]) -> Set[int]:
        """
        Collect all graph neighbors of any node in the seed, excluding nodes already in the seed itself.
        These are the only nodes to be added to the seed (only nodes that are at least adjacent to one seed node).
        """
        neighs: Set[int] = set()
        for v in seed:
            neighs.update(G.neighbors(v))
        neighs.difference_update(seed)  # rem seed nodes themselves — no add a node already in the seed
        return neighs
    ...
    # ------------------------------------------------------------------ #
    # Main loop: iterate over every original clique seed.
    # We only expand the ORIGINAL seeds, never the derived candidates.
    # ------------------------------------------------------------------ #
    for seed, s_seed in ranked:

        n_steps = max_size - len(seed)  # how many nodes can still be added to this seed?
        # if seed is already at max size, nothing to do
        if n_steps <= 0:
            continue

        neighs = _seed_neighbors(seed)  # find all neighbors reachable from any node in the seed

        if not neighs:  # if the seed has no external neighbors, we cannot expand it
            continue

        sorted_neighs = sorted(neighs)  # sort neighbors - reproducibility of combos
        seed_generated = 0

        # ------------------------------------------------------------------ #
        # For each expansion size k (1 extra node, 2 extra nodes, ...):
        # we try to add exactly k nodes from the neighbor set to the seed.
        # This directly produces candidates of size len(seed)+k,
        # always rooted at the same original seed.
        #
        # Example: seed=(A,B,C), max_size=5
        #   k=1 → candidates of size 4: (A,B,C,x) for x in neighbors
        #   k=2 → candidates of size 5: (A,B,C,x,y) for x,y in neighbors
        # ------------------------------------------------------------------ #
        for k in range(1, n_steps + 1):

            # we need at least k neighbors to form a k-subset
            if len(sorted_neighs) < k:  # not enough neighbors to add k nodes — skip this expansion size
                break

            # ------------------------------------------------------------------ #
            # Combinatorial guard:
            # C(|neighbors|, k) can grow very fast (e.g. C(100,3) = 161700).
            # If the number of possible subsets exceeds max_combinations,
            # we randomly sample max_combinations subsets instead of enumerating all.
            # This keeps runtime bounded at the cost of not exploring every candidate.
            # ------------------------------------------------------------------ #
            n_possible = math.comb(len(sorted_neighs), k)

            if n_possible > max_combinations:
                # random sampling: draw max_combinations subsets without replacement
                # each subset is a sorted tuple of k distinct neighbors
                subsets = [
                    tuple(sorted(random.sample(sorted_neighs, k)))
                    for _ in range(max_combinations)
                ]
                # remove duplicates introduced by random sampling
                subsets = list(set(subsets))
            else:
                # full enumeration: safe because n_possible <= max_combinations
                subsets = list(combinations(sorted_neighs, k))

            # ------------------------------------------------------------------ #
            # Score each candidate expansion and collect those that improve
            # enough over the seed score (by at least min_gain_ratio).
            # ------------------------------------------------------------------ #
            improvers: List[Tuple[Tuple[int, ...], float]] = []

            for node_subset in subsets:
                # build the expanded candidate by merging seed + new nodes
                # sort to get a canonical tuple (order-independent)
                cand = tuple(sorted((*seed, *node_subset)))

                # skip if cand already seen (could appear from a diff seed or a diff expansion path)
                if cand in seen:
                    skipped_seen += 1
                    continue

                # compute the community-aware score for this candidate
                s_cand = score_hyperedge(cand, U, W, kappa_mode)

                # accept only if score improves by at least min_gain_ratio
                # relative to the seed score
                if s_cand >= s_seed * (1.0 + min_gain_ratio):
                    improvers.append((cand, s_cand))
                else:
                    skipped_score += 1
            ...
            # ------------------------------------------------------------------ #
            # From all improving candidates at this expansion size k,
            # keep only the top_per_seed best ones.
            # This limits branching: we do not add every possible expansion,
            # only the most promising ones per seed and per expansion size.
            # ------------------------------------------------------------------ #
            if improvers:
                best = nlargest(
                    min(top_per_seed, len(improvers)),
                    improvers,
                    key=lambda t: t[1]
                )
                for cand, s_cand in best:
                    if cand not in seen:
                        # mark as seen to prevent future duplicates
                        seen.add(cand)
                        out.append((cand, s_cand))
                        seed_generated += 1
                        total_generated += 1

        #if seed_generated > 0:
        #    print(f"seed {seed} (size {len(seed)}, score {s_seed:.4f}) "
        #          f"→ {seed_generated} new candidates")

    print(f"\n── expand_candidates summary ──────────────────")
    print(f"  original candidates   : {len(ranked)}")
    print(f"  new candidates added  : {total_generated}")
    print(f"  total pool size       : {len(out)}")
    print(f"  skipped (seen)        : {skipped_seen}")
    print(f"  skipped (score)       : {skipped_score}")
    print(f"  seeds with sampling   : {sampled_seeds}")
    print(f"───────────────────────────────────────────────\n")

    # ------------------------------------------------------------------ #
    # Final cleanup:
    # 1. De-duplicate: if the same candidate tuple appears twice with
    #    different scores (shouldn't happen, but just in case),
    #    keep the last occurrence via dict comprehension.
    # 2. Re-rank globally by score, descending.
    # ------------------------------------------------------------------ #
    out = list({c: s for c, s in out}.items())
    out.sort(key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    R = load_correlation_matrix_R(
        'C:/Users/utente/Documents/DataScience/TESI_MAGISTRALE/DATA/GRAPHS_DIAG/POLY_mat'
    ...
    cfg = CandidateConfig(
        ...
        top_per_size=None  # keep top 200 per size; tune as needed
    )
